...
function WebOffice2015() {
	...
	this.Ashell = null;
    this.BookMark = "";						// 书签名称
	this.NoRecentFile = true;				//是否清除历史记录
    this.ImageName = "";					// 图片名称
	...
	this.INetSetCookie = function(url, cookie)
	{
		...
		this.obj.FuncExtModule.Alert("已设置Cookie");
		...
		this.WebOpen = function(mBoolean)
	{
		...
		if (this.LOADFILE(httpclient)) // Http下载服务器文件
		{
			...
			if(this.FileType != ".ppt" && this.FileType != ".pptx" && this.FileType != ".pdf" && this.FileType != ".ofd"){
				...
				// this.WebShow(false);
				this.WebSetRevision(false,true,false,false);
				// this.Alert("已保留痕迹");
			}
...
//	   	}; 
	...
	* 	删除Word的历史记录
	* */
	this.WebDelRecent = function(filePath)
	{
		for(var i=1; i<this.obj.ActiveDocument.Application.RecentFiles.Count; i++)
		{
			var tmp;
			tmp = this.obj.ActiveDocument.Application.RecentFiles.item(i).Path;
			tmp += "\\";
			tmp += this.obj.ActiveDocument.Application.RecentFiles.item(i).Name;
			if(tmp == PathName)
			{
				this.obj.ActiveDocument.Application.RecentFiles.item(i).Delete();
			}
		}
	}
...
/*	 文档保存到本地（有对话框）
	...
	this.WebOpenLocalFile = function(filePath) 
	{
		...
		if(this.blnIE()){
			...
			return 	this.obj.ExecuteScript("name","ActiveObject.Open(\""+filePath+"\")");
		}
	...
	this.getRevision = function ()
	{
		...
		for (i = 1;i <= Rev.Count;i++)
		{
			...
			if (Rev.Item(i).Type=="1"){
				Text=Text + '进行插入：'+Rev.Item(i).Range.Text+ Rev.Item(i).Date + "\r\n";
			}else if (Rev.Item(i).Type=="2"){
				Text=Text + '进行删除：'+Rev.Item(i).Range.Text+ Rev.Item(i).Date + "\r\n";
			}
			else {
				Text=Text + '进行其他操作，操作内容：“'+Rev.Item(i).Range.Text+ '”；操作：“'+Rev.Item(i).FormatDescription+ Rev.Item(i).Date +"”。\r\n";
			}
		}
    ...
    */
		...
		if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) // true 异步方式 false同步
		{
			if (httpclient.Send()) {
				...
				if (this.hiddenSaveLocal(httpclient, this, false, false)) {
					...
					// this.WebDelRecent(this.FilePath+"/"+this.FileName);
					this.WebDelRecent(this.DownloadedFileTempPathName);
	...
	this.setShowDialog = function(thisType, exts)
	{
		...
		case this.ShowDialog.DialogOpen:
			this.obj.ShowDialog(thisType, exts, 2);break; 	// 打开本地文档
	...
	this.AddWaterMark1 = function (){
		var Fun = this.obj.FuncExtModule.WebWaterMark("hahahahhaha","宋体",0xFF00FF,36,0.5,60);
		this.Alert(Fun);
	}
		...
		if(this.FileType == ".doc" || this.FileType == ".docx"){
			...
			if(this.obj.ActiveDocument.ProtectionType == "-1"){
				...
				try {
					...
					shapeRange.Name = WaterMarkNmae+i;  		//水印对象名
					shapeRange.PictureFormat.Brightness = 0.65;
					shapeRange.PictureFormat.Contrast = 0.15;
					shapeRange.Fill.Transparency = 0.5;
					shapeRange.LockAspectRatio = true;
					
					shapeRange.Height = 20 * 20; 			//高度
					shapeRange.Width = 20 * 20; 				//宽度
	...
	this.ParagraphSettings = function(){
		...
		var Selection = this.WebObject.ActiveDocument.Application.Selection;
		Selection.Font.Color = "#FFFFFF"; //字体红色
		Selection.Font.Size = 16;  //字体大小
        paragraphFormat.Alignment = 1; //常规：对齐方式,0：左对齐，1：居中，2：右对齐，以此类推
        ...
        paragraphFormat.CharacterUnitRightIndent = 10;//缩进：右侧，按照字符缩进
        ...
        paragraphFormat.LineSpacingRule = 0; // 设置行距，0：单行，1：1.5倍，2：2倍，3：最小，以此类推
	...
	this.PrintPreview = function(){
		try{
			...
			if(WebOfficeObj.FileType == ".doc"){
				WebOfficeObj.WebObject.Application.ActiveWindow.ActivePane.View.Type = 4; //打印预览
			}
			else if(WebOfficeObj.FileType == ".xls"){
				WebOfficeObj.WebObject.Application.ActiveSheet.PrintPreview();
			}
			else if(WebOfficeObj.FileType == ".wps"){
				//WebOffice.WebObject.Application.ActiveDocument.PrintPreview();
				WebOfficeObj.WebObject.Application.ActiveWindow.ActivePane.View.Type = 4;
				//alert(".wps");
			}
			else if(WebOfficeObj.FileType == ".et"){
				WebOfficeObj.WebObject.Application.ActiveSheet.PrintPreview();
				alert(".et");
			}
		}
	...
	/*删除两个标签之间的内容*/
	/*this.WebDeleteContentInBookMarks= function(bookMakr1,bookMark2){
		if(bookMakr1 && bookMark2){
			var range = this.obj.Range;
			range = this.obj.ActiveDocument.Bookmarks.Item(bookMakr1).Range;
			range.Select();
			var startR = this.obj.ActiveDocument.Application.Selection.start;
			range = this.obj.ActiveDocument.Bookmarks.Item(bookMark2).Range;
			range.select();
			var EndR= this.obj.ActiveDocument.Application.Selection.End;
			//range = this.obj.ActiveDocument.Range(Start =StartR , End = EndR);
			range = this.obj.ActiveDocument.Range(Start =StartR , End = EndR).Select();
			range.Delete();
		}
	}*/
	//删除两个标签之间的内容
	this.WebDeleteContentInBookMarks=function(bookMakr1,bookMark2){
		if(bookMakr1&&bookMark2){
			if(this.obj.ActiveDocument.BookMarks.Exists(bookMakr1)){
				var range=this.obj.Range;
				range=this.obj.ActiveDocument.Bookmarks.Item(bookMakr1).Range;
				range.Select();
				var StartR=this.obj.ActiveDocument.Application.Selection.Start;
				...
				console.log("域'"+bookMakr1+"'不存在");
			}
			if(this.obj.ActiveDocument.BookMarks.Exists(bookMark2))//判断是否存在该书签,删除动态添加的书签
			{
				this.WebDelBookMarks(bookMark2);
			}
		}
	}
